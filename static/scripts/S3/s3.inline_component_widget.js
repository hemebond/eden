...
function S3InlineComponentWidgetAddClick(Args)
{
	...
	for (i= 0; i < Args.ColumnFields.length; i++) {
		...
		if (InputSelector.is("select") )
			...
			if (InputSelector.attr("class") == "double")
				{
				DisplayValue = DisplayValue.replace(/[$,]/g,"");
				if (DisplayValue) {
	...
	testJSONInputValue = newJSONInputValue.replace(/[\)]/g,"\\\)")
	testJSONInputValue = testJSONInputValue.replace(/[\(]/g,"\\\(")
	testJSONInputValue = testJSONInputValue.replace(/[\]]/g,"\\\]")
	testJSONInputValue = testJSONInputValue.replace(/[\[]/g,"\\\[")
	//Test: if there is data in the input fields, and it does not match data whcih has already been added
	if (!InputBlank && !JSONInputValue.match(testJSONInputValue)) {
		if (JSONInputValue != "")
		{
		...
		if (FileInputID != "") {
			...
			$('<input class="upload" id="' + FileInputID.substring(1) +'" type="file" />').insertAfter("#" + NewFileInputID)
...
function S3InlineComponentWidgetDeleteClick(WidgetDelete, Args)
{
	var WidgetID = Args.WidgetID;
